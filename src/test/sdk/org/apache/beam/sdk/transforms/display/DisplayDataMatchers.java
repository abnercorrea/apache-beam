...
public class DisplayDataMatchers {
  ...
  public static Matcher<DisplayData> hasDisplayItem() {
    return new FeatureMatcher<DisplayData, Collection<Item>>(
   ...
   */
  public static Matcher<DisplayData> hasDisplayItem(Matcher<Item> itemMatcher) {
  ...
  private static class HasDisplayDataItemMatcher extends TypeSafeDiagnosingMatcher<DisplayData> {
    ...
    private HasDisplayDataItemMatcher(Matcher<Item> itemMatcher) {
  ...
  /**
   * Creates a matcher that matches if the examined {@link Item} contains a key with the
   * specified value.
   */
  public static Matcher<Item> hasKey(String key) {
    return hasKey(is(key));
  }

  /**
   * Creates a matcher that matches if the examined {@link Item} contains a key matching
   * the specified key matcher.
   */
  public static Matcher<Item> hasKey(Matcher<String> keyMatcher) {
    return new FeatureMatcher<Item, String>(keyMatcher, "with key", "key") {
      @Override
  ...
  /**
   * Creates a matcher that matches if the examined {@link Item} contains a path
   * matching the specified namespace.
   */
  public static Matcher<Item> hasPath(String... paths) {
    DisplayData.Path path =
        (paths.length == 0)
            ? DisplayData.Path.root()
            : DisplayData.Path.absolute(paths[0], Arrays.copyOfRange(paths, 1, paths.length));
    return new FeatureMatcher<Item, DisplayData.Path>(
        is(path), " with namespace", "namespace") {
      @Override
      protected DisplayData.Path featureValueOf(Item actual) {
  ...
  /**
   * Creates a matcher that matches if the examined {@link Item} contains the specified
   * namespace.
   */
  public static Matcher<Item> hasNamespace(Class<?> namespace) {
    return hasNamespace(Matchers.<Class<?>>is(namespace));
  }

  /**
   * Creates a matcher that matches if the examined {@link Item} contains a namespace
   * matching the specified namespace matcher.
   */
  public static Matcher<Item> hasNamespace(Matcher<Class<?>> namespaceMatcher) {
    return new FeatureMatcher<Item, Class<?>>(
        namespaceMatcher, " with namespace", "namespace") {
      @Override
      protected Class<?> featureValueOf(Item actual) {
  ...
  /**
   * Creates a matcher that matches if the examined {@link Item} matches the specified
   * type.
   */
  public static Matcher<Item> hasType(DisplayData.Type type) {
    return hasType(is(type));
  }

  /**
   * Creates a matcher that matches if the examined {@link Item} has a type matching the
   * specified type matcher.
   */
  public static Matcher<Item> hasType(Matcher<DisplayData.Type> typeMatcher) {
    return new FeatureMatcher<Item, DisplayData.Type>(
        typeMatcher, "with type", "type") {
      @Override
      protected DisplayData.Type featureValueOf(Item actual) {
  ...
  /**
   * Creates a matcher that matches if the examined {@link Item} has the specified
   * value.
   */
  public static Matcher<Item> hasValue(Object value) {
    return hasValue(is(value));
  }

  /**
   * Creates a matcher that matches if the examined {@link Item} contains a value
   * matching the specified value matcher.
   */
  public static <T> Matcher<Item> hasValue(Matcher<T> valueMatcher) {
    return new FeatureMatcher<Item, T>(valueMatcher, "with value", "value") {
      @Override
      protected T featureValueOf(Item actual) {
   ...
   * label.
   */
  public static Matcher<Item> hasLabel(String label) {
    return hasLabel(is(label));
  }

  /**
   * Creates a matcher that matches if the examined {@link Item} has a label matching
   * the specified label matcher.
   */
  public static Matcher<Item> hasLabel(Matcher<String> labelMatcher) {
